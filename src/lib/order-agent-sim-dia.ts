 ...
 *
 * Built from scratch 2026-04-27 (NOT a SPY clone). DIA trades around $425 vs
 * SPY ~$560 (~0.76x). DIA ATM weeklies typically run $2.00-4.50 with somewhat
 * thinner liquidity than SPY. Premium floor multiplier set to 4x — between
 * SPY's 5x (high liquidity) and the shared 3x default (single-stock baseline).
 * Calibrate from live fills once available.
 */
...
export function simulateOrderAgentDia(
  ...
  const { entryPremium, recentVolatility, optionAtr } = estimateEntryPremium(
    atr, delta, cfg.recentBars, DIA_PREMIUM_FLOOR_MULT,
  );
