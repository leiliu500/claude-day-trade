/**
 * order-agent-sim-nvda.ts — NVDA-specific order simulation.
 *
 * NVDA weekly options are higher-premium than 0DTE ETFs.
 * Uses 3x premium floor (standard) — NVDA ATM weeklies are expensive
 * enough that the default multiplier is appropriate.
 */
...
const NVDA_PREMIUM_FLOOR_MULT = 3;

export function simulateOrderAgentNvda(
  entryPrice: number,
  ...
  const { entryPremium, recentVolatility, optionAtr } = estimateEntryPremium(
    atr, delta, cfg.recentBars, NVDA_PREMIUM_FLOOR_MULT,
  );
