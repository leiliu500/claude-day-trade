...
export function isSpyAfternoonFlowContinuation(input: SpyAfternoonContinuationInput): boolean {
  ...
  if (input.displacementVelocity === undefined
      || input.displacementVelocity > 0.10
      || input.displacementVelocity < -0.05) return false;
  if (input.atrRatio === undefined || input.atrRatio > 1.0) return false;

  const b = input.breakdown;
  const common = b.trendPersistenceBonus >= 0.06
    ...
    && b.adxBonus >= 0.05
    && b.macdBonus >= 0.03
    && b.volumeSurgeBonus <= 0.04
    && b.nearLevelPenalty >= -0.05
    && b.moveExhaustionPenalty >= -0.05;
  if (!common) return false;

  // Two flow shapes: a candle/PV pulse with moderate-to-strong order flow,
  // OR a flow reacceleration without a fresh pulse.
  const flowPulse = b.candlePatternBonus >= 0.02
    && b.candlePatternBonus <= 0.06
    && b.priceVelocityBonus >= 0
    && b.orderFlowBonus >= 0.04
    && b.orderFlowBonus <= 0.20;
  const flowReacceleration = b.candlePatternBonus <= 0.02
    && b.orderFlowBonus >= 0.15;

  return flowPulse || flowReacceleration;
}
