 ...
 *
 * Baseline cloned from order-agent-sim-spy.ts on 2026-04-22 — SPY's dynamic
 * delta simulation (premium floor, gamma, theta, trailing floor) is the
 * starting point for tuning QQQ. Replace these constants as QQQ-specific
 * live-fill calibration data becomes available.
 */
...
// Premium floor multiplier (cloned from SPY baseline).
const QQQ_PREMIUM_FLOOR_MULT = 5;

// Default gamma for 0DTE/1DTE options (cloned from SPY baseline).
const QQQ_DEFAULT_GAMMA = 0.008;

// Default theta: ~0.10% of premium per minute (cloned from SPY baseline).
const QQQ_DEFAULT_THETA_PER_MIN_PCT = 0.001;
...
export function simulateOrderAgentQqq(
  ...
  const gamma = cfg.gamma ?? QQQ_DEFAULT_GAMMA;
  const trace = cfg.trace ?? false;

  const { entryPremium, recentVolatility, optionAtr } = estimateEntryPremium(
    atr, entryDelta, cfg.recentBars, QQQ_PREMIUM_FLOOR_MULT,
  );

  const thetaPerMin = cfg.theta ?? (entryPremium * QQQ_DEFAULT_THETA_PER_MIN_PCT);
