/**
 * order-agent-sim-aapl.ts — AAPL-specific order simulation.
 *
 * AAPL weekly options have moderate premiums — between ETF 0DTEs and NVDA weeklies.
 * Uses 3x premium floor (standard) — AAPL ATM weeklies are liquid enough
 * that the default multiplier is appropriate.
 */
...
const AAPL_PREMIUM_FLOOR_MULT = 3;

export function simulateOrderAgentAapl(
  entryPrice: number,
  ...
  const { entryPremium, recentVolatility, optionAtr } = estimateEntryPremium(
    atr, delta, cfg.recentBars, AAPL_PREMIUM_FLOOR_MULT,
  );
